  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative overflow-hidden border-b border-border/60">
        ...
        <div className="relative z-10 mx-auto max-w-4xl px-4 pb-16 pt-12 text-center sm:px-6 sm:pb-24 sm:pt-20">
          <p className="mb-4 font-mono text-sm uppercase tracking-[0.2em] text-signal-green sm:mb-5 sm:text-base">
            / the oracle
          </p>
          <h1 className="text-balance text-4xl font-semibold leading-[1.05] tracking-tight sm:text-6xl">
            <span className="bg-gradient-to-b from-foreground via-foreground to-foreground/75 bg-clip-text text-transparent">
              A weekly read on what the smart money is pricing.
            </span>
          </h1>
          <p className="mx-auto mt-5 max-w-2xl text-balance text-base text-muted-foreground sm:mt-6 sm:text-lg">
            Every Sunday morning. One tight briefing on the biggest whale moves of the
            week, the cross-venue spreads worth watching, and the catalysts on next
            week&apos;s tape. Free, no fluff, unsubscribe anytime.
          </p>
      ...
      <section className="border-b border-border/60 py-14 sm:py-24">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          ...
          <div className="grid gap-px overflow-hidden rounded-lg border border-border bg-border sm:grid-cols-2">
            {SECTIONS.map((s) => (
              <div
                key={s.tag}
                className="flex flex-col gap-2.5 bg-background p-5 transition-colors hover:bg-muted/30 sm:gap-3 sm:p-7"
              >
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          ...
          <ul className="divide-y divide-border/60 border-y border-border/60">
            {PAST_ISSUES.map((issue) => (
              <li key={issue.n} className="flex flex-col gap-1.5 py-5 sm:flex-row sm:items-baseline sm:gap-6 sm:py-6">
                <div className="flex flex-shrink-0 items-baseline gap-3 font-mono text-xs text-muted-foreground sm:w-44">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">{issue.blurb}</p>
                </div>
              </li>
            ))}
          </ul>
...
function PageBackground() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden>
      ...
      <div className="absolute inset-x-0 top-0 h-[1px] animate-scan-y-slow bg-gradient-to-r from-transparent via-signal-green/30 to-transparent" />
